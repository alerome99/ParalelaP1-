 ...
 */
	...
	for( iter=0; iter<max_iter && current_max_food <= max_food && num_cells_alive > 0; iter++ ) {
		...
		/*
		float *temporal = (float *)malloc( sizeof(float) * num_cells );
		for (i=0; i<num_new_sources; i++) {
			temporal[i] = erand48( food_random_seq );
		}
		j = 0;
		*/
		/*
		for (i=0; i<num_new_sources; i++) {
		  row = (int)temporal[j];
			...
			col = (int)temporal[j];
			j++;
			food = (float)(food_level * temporal[j]);
			j++;
		...
		for (i=0; i<num_new_sources; i++) {
			...
			foodVec[i] = (float)( food_level * erand48( food_random_seq ));
			posVec[i] = row*columns+col;
		}

		for (i=0; i<num_new_sources; i++){
			culture[posVec[i]] = culture[posVec[i]] + foodVec[i];
		}
		...
		if ( food_spot_active ) {
			for (i=0; i<num_new_sources; i++) {
				culture[posVec2[i]] = culture[posVec2[i]] + foodVec2[i];
			}
		}
		...
		/*#pragma omp parallel for default(none) private(i, j) firstprivate(rows, columns) shared(culture_cells)*/
		...
		for(i=0; i<num_cells; i++){
			if ( cells[i].alive ) {
				...
				if ( cells[i].storage < 1.0f ) {
					...
					temporal[i] = (float)erand48( cells[i].random_seq );
				}
			}
		}
		*/
		tiempoBucle3 = cp_Wtime();
		int auxiliar = sim_stat.history_max_age;
		#pragma omp parallel for firstprivate(num_cells) reduction(max:auxiliar) private(i) reduction(+:num_cells_alive, step_dead_cells) shared(food_to_share, culture_cells, culture, cells, columns, rows)
		/* 4.3. Cell movements */
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
				cells[i].age ++;
				// Statistics: Max age of a cell in the simulation history
				if ( cells[i].age > auxiliar )
					auxiliar = cells[i].age;
				...
				if ( cells[i].storage < 1.0f ) {
					...
					// else do not ch	ange the direction
		...
		sim_stat.history_max_age = auxiliar;
		...
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
				...
				if ( cells[i].age > 30 && cells[i].storage > 20 ) {
					temporal[i] = (unsigned short) nrand48( cells[i].random_seq );
				}
			}
		}*/
		/*
		int auxiliar2;
		j = 0;
		//int *temporal = (int *)malloc( sizeof(int) * num_cells );
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
				auxiliar2 = cells[i].storage;
				//auxiliar2 += (food_to_share[i])/(accessMat( culture_cells, cells[i].pos_row, cells[i].pos_col ));
				if ( cells[i].age > 30 && auxiliar2 > 20 ) {
					step_new_cells++;
					//temporal[j] = step_new_cells;
					j++;
				}
			}
		}
		j = 0;*/
		/*
		int auxiliarVar = step_new_cells;
		int *temporal = (int *)malloc( sizeof(int) * num_cells );
		#pragma omp parallel for
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
		...
		//printf("%d",j);
		//printf("\n");
		int prueba2 = sim_stat.history_total_cells;
		#pragma omp parallel for reduction(+:num_cells_alive, prueba2) private(i) reduction(max:step_new_cells)
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
				// 4.4.1. Food harvesting
				...
				if ( cells[i].age > 30  && cells[i].storage > 20) {
					...
					num_cells_alive ++;
					prueba2 ++;
					//if(j<sizeof(temporal)){
					step_new_cells = temporal[i];
					//printf("\n");
					//printf("hola %ld hola2 %ld", s, p);
					//}
					// New cell is a copy of parent cell
		...
		sim_stat.history_total_cells = prueba2;
		*/
		//int auxiliarSum = step_new_cells;
		/*
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
				...
				if ( cells[i].age > 30 && cells[i].storage > 20 ) {
					...
					prueba2 ++;
					int auxiliarSum = 10;
					new_cells[ auxiliarSum-1 ] = cells[i];

					cells[i].storage /= 2.0f;
					new_cells[ auxiliarSum-1 ].storage /= 2.0f;
					cells[i].age = 1;
					new_cells[ auxiliarSum-1 ].age = 1;

					new_cells[ auxiliarSum-1 ].random_seq[0] = (unsigned short) nrand48( cells[i].random_seq );
					new_cells[ auxiliarSum-1 ].random_seq[1] = (unsigned short) nrand48( cells[i].random_seq );
					new_cells[ auxiliarSum-1 ].random_seq[2] = (unsigned short) nrand48( cells[i].random_seq );

					cell_new_direction( &cells[i] );
					cell_new_direction( &new_cells[ auxiliarSum-1 ] );

					cell_mutation( &cells[i] );
					cell_mutation( &new_cells[ auxiliarSum-1 ] );
				}
			}
		}  */
		...
		/*#pragma omp parallel for default(none) private(i) firstprivate(num_cells, columns) shared(cells, culture)*/
		//omp_set_num_threads(2);
		//#pragma omp parallel for
		for (i=0; i<num_cells; i++) {
			if ( cells[i].alive ) {
		...
		#pragma omp parallel for default(none) shared(culture) private(i) firstprivate(rows, columns) reduction(max:current_max_food)
		for( i=0; i<rows * columns; i++ ){
			culture[i] *= 0.95f;
		...
		/*
		#pragma omp parallel for default(none) shared(culture) private(i, j) firstprivate(rows, columns) reduction(max:current_max_food)
		for( i=0; i<rows; i++ )
			for( j=0; j<columns; j++ ) {
